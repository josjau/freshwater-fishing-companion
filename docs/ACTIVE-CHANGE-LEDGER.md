...
Fish Guide Phase 0 is closed. FISH-001 through FISH-007 are terminal historical items and are no longer active ledger entries. Trout Production Package 1, Gar Production Package, Production Wave 1 (Common Carp + Freshwater Drum + Paddlefish), and Production Wave 2 (Walleye / Sauger + Catfish) are closed. Production Wave 3 — Bass has completed source/package validation and user browser approval in the Drive working package. The only remaining Wave 3 closeout gates are applying the final documentation-closeout increment to the local Git checkout, explicit commit/push authorization, and post-push GitHub verification.

| ID | Status | Item | Canonical detail / gate | Next action |
|---|---|---|---|---|
| FISH-008 | APPROVED / PENDING IMPLEMENTATION | Locked Fish production architecture — remaining library | D057–D061 + `data-model/02-FISH.md` + `data-model/09-RELATIONSHIPS.md` + `FISH_REFERENCE_SOURCES.md` + `workstreams/FISH-WAVE-3-BASS.md` | Commit/push and re-fetch the user-approved Wave 3 Bass package, then proceed with Sunfish & Crappie unless a genuine evidence/media/relationship/product decision requires a smaller boundary. The status remains library-wide because Fish outside Wave 3 are still pending. |
| FISH-009 | APPROVED / PENDING IMPLEMENTATION | Locked Fish UX — remaining library | Fish Phase 0 archive + D051/D061 + `workstreams/FISH-WAVE-3-BASS.md` + validated Trout/Gar/Wave 1/Wave 2 presentation | Wave 3 visual review is complete: Compare Similar Fish centering, Selection/Fish ID image blocks, native `→`, and the shared `font-weight: 800` treatment for `←`, `→`, `↗`, and compact-row `›` are user-approved. Commit/push and re-fetch the approved package, then continue using this validated Fish/navigation presentation baseline for later packages. |
...
**Current Wave 3 checkpoint:** All six processed images, the 12-recommendation Fish-to-Rig set, all six Fish records, all six deterministic comparison pairs, rights/provenance treatment, per-context image framing, Compare Similar Fish centering, Selection/Fish ID image blocks, the native Unicode `→` treatment, and the shared `font-weight: 800` navigation-arrow treatment are user-approved. Required source/package validation passes. The set remains uncommitted and unpushed pending the final documentation-closeout increment, explicit commit/push authorization, and post-push GitHub verification. While the affected source files were open, obsolete package-era build labels were removed from the four Fish/Media registries and `view-renderer.js` under UX-008; UX-008 remains active for other affected permanent source files when they are deliberately reopened.
