...
When a secondary workflow, card, recommendation surface, or other UI reuses an already-canonical entity/category/collection identity, it must source the visible title from that canonical owner rather than maintain a separately authored duplicate title. Supporting/contextual copy may vary by surface, but the canonical user-facing name does not.

When the same canonical peer set also has an established canonical display order, secondary surfaces should derive/reuse that order rather than maintain a second local ordering list unless an explicitly approved task-specific ordering rule requires an exception. Workflow-only options that are not part of the canonical peer set may be inserted separately (for example, a local general-purpose choice before the canonical categories).

This is a consistency/data-ownership rule as well as a presentation rule: a future canonical rename or reorder must propagate to dependent surfaces without requiring a second manual title/order edit. Exact source wiring remains subject to the owning domain's structural audit, but intentionally duplicated labels/order are not the default implementation.
...
### Guide-Family Browse/Collection Card Inheritance

The validated Fish browse-card grammar is a **Guide-family baseline**, not a Fish-only convention. Every later Guide audit (including Knots, Tackle, Technique, Rig, and comparable future Guides) must begin equivalent collection/browse cards from this treatment and explicitly verify it rather than relying on chat memory or rediscovering the pattern. A deliberate exception is allowed only when the Guide's semantics require it and the audit records the reason.

For an equivalent Guide-family collection/browse card:

- the collection title and explicit **`Browse →`** action cue share the heading row when space allows;
- the short collection description follows below the heading row;
- the whole card is the interactive control and the action text is explanatory, not a nested competing destination;
- the action uses the approved lighter non-pill text treatment;
- when title + action cannot fit comfortably, move the complete **`Browse →`** cue to a new **left-aligned** line rather than squeezing the title, shrinking text, or right-aligning the wrapped action;
- normal collection cards continue to use the shared standard-card accent system and must not consume reserved special/workflow accents; and
- each Guide-specific audit file must carry an explicit compliance/exception item for this grammar through implementation and browser validation.
...
- expose a visible position cue such as **1 of 3**;
- provide explicit **Previous / Next** controls whenever another page exists;
- treat left/right swipe as an optional touch enhancement only, never the sole navigation method;
- never autoplay between pages;
