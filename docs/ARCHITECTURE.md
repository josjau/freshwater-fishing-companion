...
- `view-renderer.js` owns reusable rendering, shared search/result UI, catalog/detail rendering, contextual Tackle references, derived relationship navigation, readiness presentation, sticky Parent/Home controls, modal focus restoration, and the explicit structural mount points where feature-specific renderers integrate with a page.
- `knot-media-renderer.js` owns Knot instructional-media presentation. It integrates through an explicit Knot Detail media call/mount point owned by `view-renderer.js`; it must not silently replace/reassign a renderer declared in another file as its normal integration mechanism. Canonical Knot instructions remain owned by `data/knots.js`, and Media records/provenance remain owned by `data/media.js`.
...
- Cross-Guide navigation/handoff helpers are owned by the **originating Guide or feature** when they capture origin-specific state, return context, or transition behavior; the destination Guide owns its own destination behavior. Only genuinely symmetric/infrastructure-level handoff machinery belongs in shared/common ownership.
...
`PROJECT-RULES.md` is the single canonical current implementation procedure. This Architecture document records technical/source architecture and durable ownership boundaries only; startup, approval, editing/transport, production, review/package, commit/push, validation, closeout, failure-recovery, Guide-audit, and rule-lifecycle mechanics belong to `PROJECT-RULES.md`.