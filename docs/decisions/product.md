...
**Current implementation status:** GATE-004 is ACTIVE / REQUIRED from GitHub `main` baseline `7aab1bbbbfb6837efbb175d715f55f9146fdad39`. G4-CAND, G4-EQUIP, G4-SIMPLE, G4-LEGAL, and G4-CTX are COMPLETE / APPROVED WITH REVISION ALLOWED at the semantic-contract level. The five-item Recommendation semantic handoff is complete; production implementation now proceeds only after the planning-to-build documentation closeout passes, beginning with the Recommendation Decision Knowledge production model/runtime boundary.
...
Recommendation may consume authoritative structured regulatory constraints but does not author, scrape, or infer legal requirements from jurisdiction names, Fish identity, descriptive official-resource links, User Knowledge, or indirect context. The current D066 Regulations feature remains an official-resource gateway and is not machine-readable legal authority. Candidate legal evaluation distinguishes **Not Evaluated**, **No Known Blocking Constraint**, **Blocked by Known Constraint**, and **Compliance Unconfirmed**; FCC does not use a blanket `Legal` state. Lack of structured regulatory coverage does not prevent otherwise valid candidates from participating in Best Overall or Best Currently Available, but FCC must clearly disclose that regulations were not evaluated and provide the applicable official-resource path where available. A known authoritative prohibition is a hard exclusion. An applicable legal requirement may become a hard candidate parameter, while G4-EQUIP separately determines whether confirmed current equipment proves that requirement. Legal evaluation requires explicit applicable jurisdiction/context and fails safely when rule applicability or freshness cannot be established.
