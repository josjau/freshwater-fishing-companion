...
**Persistence/authentication carry-forward is RESOLVED for Version 1 beginner-workflow continuity, with refinement allowed.** Choose a Setup / Start Here workflow draft state, a resolved saved setup plan, and Starter Tackle Box build/checklist progress may persist **device-locally across sessions** without authentication. The user must be able to explicitly continue, reset, or clear that retained local state. Local workflow persistence is convenience/plan state, **not authoritative User Knowledge** and not proof of ownership. A recommendation, saved setup decision, or checklist completion must never silently create My Tackle ownership. When the user explicitly confirms that an item is owned or chooses an Add/Mark Owned action, that ownership crosses into authoritative My Tackle and uses the shared synchronized User Knowledge architecture. Cross-device saved plans/progress are not required for the Version 1 Choose a Setup / Starter Tackle Box experience; if added later, they become profile-owned User Knowledge and depend on the shared authentication/synchronization platform.
...
The structured scope should include only legal constraints that materially affect Recommendation candidates. Comprehensive reproduction of changing fishing law is not required. **Structured Recommendation Legality is not a universal hard prerequisite for a useful Version 1 What Should I Throw experience.** Lack of structured regulatory coverage does not prevent otherwise valid candidates from participating in Best Overall or Best Currently Available, but the UI must never equate unevaluated legality with confirmed legality: the evaluation state must be explicit and the applicable official Regulations resource path must be available. A known authoritative prohibition is a hard exclusion. An applicable legal requirement may become a hard candidate parameter, while G4-EQUIP separately determines whether confirmed current equipment proves that requirement. Legal evaluation requires explicit applicable jurisdiction/context and fails safely when rule applicability or freshness cannot be established. The 2026-09-19 FCC 46B four-state dependency challenge found the higher-risk Recommendation-legality cases concentrated primarily in special-regulation waters/methods such as managed trout waters, Paddlefish/snagging, live-bait restrictions, and unusual hook/multi-lure constraints rather than ordinary primary-species rod-and-reel fishing. Those higher-risk contexts may require stronger structured coverage before FCC presents an unqualified legal recommendation. Exact constraint schema, authored coverage, evidence/freshness policy, maintenance workflow, candidate behavior, and UX remain G4-LEGAL implementation work.
